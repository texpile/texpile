...
import { FIXTURES, applyOp } from './paper-scenarios.mjs';
...
// MATRIX_PICK=name1,name2 runs only those scenarios (targeted re-verification)
...
const rows = [];
// stream rows as they land: a 222-scenario run must not lose everything to one crash
...
try {
	...
	for (const fx of FIXTURES) {
		...
		for (let si = 0; si < fx.scenarios.length; si++) {
			const sc = fx.scenarios[si];
			...
			try {
				...
				if (d.kind === 'noop') res = { outcome: 'NOOP', latencyMs: 0, reasons: [] };
					...
					res = classify(ev);
					// the engine grading its own live render: patch-verify fires when the
					// reconcile lands, measuring how far the patched rows drifted from truth
				...
				retried.add(sc.name);
				console.log(`  (harness page died on ${sc.name}; rebooting and retrying)`);
				try {
					await page.close();
					page = await browser.newPage({ viewport: { width: 960, height: 1000 } });
				} catch {
					// the whole browser is gone: relaunch it
				...
				await page.waitForFunction(() => !!window.__live, undefined, { timeout: 60000 });
				await page.evaluate((r) => window.__live.open(r), root);
				await collectUntilQuiet(page, { quietMs: 1500, maxMs: 60000 });
				si--;
			}
		}
		await post('/stop', {});
