...
7. A theme colour never goes straight onto the ground. As text or a thin mark it is `text-{family}-ink` (in CSS `var(--{family}-ink)`), whose lightness is clamped per mode so it reads on a pastel scale too. As a tint behind content it is `bg-{family}-tint` (`-wash` for hover and hairlines, `-flood` for pressed or selected), never `bg-{family}-500/15`: Tailwind bakes an alpha-modified family colour to Skeleton's base placeholder, a grey that ignores the theme. The neutral pair has `surface-tint`, `surface-wash`, `surface-strong-wash` and `surface-scrim`. Text on a fill is Skeleton's `text-{family}-contrast-500`, never `text-white`. `text-primary-500` on a surface is the shape Reign turns invisible
8. Presets follow Skeleton's own landing recipe. One `preset-filled-primary-500` per surface, on the action that surface exists for, and nothing else in primary. Every other button is `preset-outlined-surface-200-800 hover:preset-tonal`. Hover is always the neutral `hover:preset-tonal`; primary marks a selected state, never a hover. `preset-tonal-{family}` is for badges and selected states, not for buttons. This is the whole reason their site holds up on every theme. One deliberate exception: the compile slot is a tonal in its state colour with a hairline of its own hue, so it reads as Texpile's control and not as another toolbar button
9. A control is a Skeleton class before it is anything else: `btn` / `btn-icon` with a size, `chip`, `card`, `badge`, `input`, `select`, `textarea`, `checkbox`. They carry the radius, padding, icon sizing and hover, and their docs site is built from them and nothing else. A `<button>` with its own `rounded-base px-2 py-1` is a control the theme cannot reach. The exceptions are menu rows, which Skeleton has no class for, and fixed-height title-bar chrome
10. Two radii, by what the box is. `rounded-base` is for controls that hold one line: buttons, inputs, chips, menu rows. `rounded-container` is for anything that holds content: cards, panels, popovers, menus, code boxes, and every textarea. A theme may set the base radius to a pill (Rosé Pine does), and a pill on a tall box swallows its corners
11. Skeleton's presets are the test: every one of them must render correctly with no changes. Something that looks wrong under a preset is fixed in the tokens, never with a special case for that theme
